...
<html>
	...
	<body>
		...
		<?php
			...
			$sql = 'SELECT u.id,c.title,u.username,u.phone,u.gender,u.pass_time,u.pic,s.chinese,s.math,s.english FROM users u,classes c,score s WHERE u.cid = c.id && u.id = s.uid '.$where.' ORDER BY u.id DESC';
			$data = DB($sql);
...
//				?>
				<tr>
					...
					<td><?php $pic = $v['pic']; echo "<img style='width: 50px;height: 50px' src = '$pic'>" ?></td>
					<td><a href="modUser.php?id=<?php echo $v['id'] ?>">修改</a> | <a href="del.php?id=<?php echo $v['id'] ?>&act=delUser">删除</a></td>
				</tr>
			<?php endforeach; ?>
			<?php if(!isset($_GET['search'])): ?>
				<tr>
					<td colspan="12">总人数:<?php echo $countUser[0]['count'] ?>　
